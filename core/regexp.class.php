...
class miniRegexp {
    ...
    // Check to parse arrow from fishes when player has disconnected
    const DISCONNECT_PLAYER_FISH = '<-';
    /** 
    ...
    */
    ...
    public function isDisconnectedPlayerFish($name) {
        if (strpos($name, self::DISCONNECT_PLAYER_FISH) === 0) {
            return true;
        }
        return false;
    }
...
/*                case ($this->getStage() === self::RESULTS && $this->isDisqualified($r)):
                ...
                case ($this->getStage() === self::FISHES && $this->isBiggestFish($r) === false) :
                    ...
                    if (is_array($playerName)) {
                        if ($this->isDisconnectedPlayerFish($playerName[1])) {
                            $plrName = preg_replace('/\<\-/', '', $playerName[1], 1);
                            $isDisconnect = true;
                        } else {
                            $plrName = $playerName[1];
                        }
                    } else {
                        if ($this->isDisconnectedPlayerFish($playerName)) {
                            $plrName = preg_replace('/\<\-/', '', $playerName, 1);
                            $isDisconnect = true;
                        } else {
                            $plrName = $playerName;
                        }
